...
-- 社团分类演示数据
-- ----------------------------
INSERT INTO club_category (category_code, category_name, sort_order, create_time, create_by) VALUES
('technology', '科技类', 1, NOW(), 'admin'),
('art', '艺术类', 2, NOW(), 'admin'),
('sports', '体育类', 3, NOW(), 'admin'),
('academic', '学术类', 4, NOW(), 'admin'),
('volunteer', '志愿类', 5, NOW(), 'admin'),
('test_234811', 'test_234811', 999, NOW(), 'admin')
...
(1872600000000000001, '编程魔法师', 'coding_wizards', (SELECT category_id FROM club_category WHERE category_code = 'technology' AND del_flag = '0' LIMIT 1), '开发者分享知识、构建项目并参加黑客马拉松的社区。我们每周聚会一次，讨论最新的技术，开展开源项目，并为编程面试做准备。', 'https://images.unsplash.com/photo-1531482615713-2afd69097998?auto=format&fit=crop&q=80&w=800', 128, 102, '张明', '李华', '2020-09-01', '科技楼 304室', '每周三 18:00', NOW(), 'admin'),
(1872600000000000002, '辩论协会', 'debate_society', (SELECT category_id FROM club_category WHERE category_code = 'academic' AND del_flag = '0' LIMIT 1), '通过每周对当前全球问题的辩论，磨练你的修辞和批判性思维能力。', 'https://images.unsplash.com/photo-1524178232363-1fb2b075b655?auto=format&fit=crop&q=80&w=800', 45, NULL, '鲍勃·史密斯', '', '2018-03-15', '学生活动中心 101', '每周二 17:00', NOW(), 'admin'),
(1872600000000000003, '摄影俱乐部', 'photo_club', (SELECT category_id FROM club_category WHERE category_code = 'art' AND del_flag = '0' LIMIT 1), '捕捉你周围的世界。每周摄影采风、工作坊和展览。', 'https://images.unsplash.com/photo-1542038784456-1ea8e935640e?auto=format&fit=crop&q=80&w=800', 82, NULL, '查理·布朗', '', '2019-11-20', '艺术工作室 B', '每周四 16:30', NOW(), 'admin'),
(1872600000000000004, '绿色地球', 'green_earth', (SELECT category_id FROM club_category WHERE category_code = 'volunteer' AND del_flag = '0' LIMIT 1), '通过校园倡议促进可持续发展和环境意识。', 'https://images.unsplash.com/photo-1542601906990-b4d3fb778b09?auto=format&fit=crop&q=80&w=800', 200, NULL, '戴安娜', '', '2015-04-22', '社区花园', '每周五 15:00', NOW(), 'admin'),
(1872600000000000005, '乐器合奏团', 'music_ensemble', (SELECT category_id FROM club_category WHERE category_code = 'art' AND del_flag = '0' LIMIT 1), '加入其他音乐家进行即兴演奏、表演和音乐鉴赏之夜。', 'https://images.unsplash.com/photo-1511379938547-c1f69419868d?auto=format&fit=crop&q=80&w=800', 65, NULL, '埃文', '', '2016-09-10', '音乐厅', '每周一 19:00', NOW(), 'admin'),
(1872600000000000006, '机器人俱乐部', 'robotics_club', (SELECT category_id FROM club_category WHERE category_code = 'technology' AND del_flag = '0' LIMIT 1), '建造和编程机器人。参加全国比赛并学习机电一体化。', 'https://images.unsplash.com/photo-1561557944-6e7860d1a7eb?auto=format&fit=crop&q=80&w=800', 50, NULL, '菲奥娜', '', '2021-01-15', '工程实验室', '每周六 10:00', NOW(), 'admin'),
(1872600000000000007, 'AI 研习社', 'ai_study_group', (SELECT category_id FROM club_category WHERE category_code = 'technology' AND del_flag = '0' LIMIT 1), '探索人工智能的前沿技术，包括深度学习、计算机视觉和自然语言处理。', 'https://plus.unsplash.com/premium_photo-1683121710572-7723bd2e235d?auto=format&fit=crop&q=80&w=800', 150, NULL, 'GPT-4', '', '2023-05-01', '创新中心 202', '每周五 14:00', NOW(), 'admin'),
(1872600000000000008, '篮球社', 'basketball_club', (SELECT category_id FROM club_category WHERE category_code = 'sports' AND del_flag = '0' LIMIT 1), '热爱篮球的同学集结地，每周组织训练和校内友谊赛。', 'https://images.unsplash.com/photo-1546519638-68e109498ee2?auto=format&fit=crop&q=80&w=800', 90, NULL, '科比·粉丝', '', '2010-09-01', '北区篮球场', '每周二、四 18:00', NOW(), 'admin');

-- ----------------------------
-- 分类ID同步（兼容历史数据中 club.category_id 与分类主键不一致）
-- ----------------------------
UPDATE club c
JOIN (
  SELECT 1 AS legacy_category_id, 'technology' AS category_code
  UNION ALL SELECT 2, 'art'
  UNION ALL SELECT 3, 'sports'
  UNION ALL SELECT 4, 'academic'
  UNION ALL SELECT 5, 'volunteer'
) m ON c.category_id = m.legacy_category_id
JOIN club_category cc ON cc.category_code = m.category_code AND cc.del_flag = '0'
SET c.category_id = cc.category_id;
